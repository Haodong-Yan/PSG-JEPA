...
The encoder is pretrained on LIBERO-Goal demonstrations with physical state grounding, a
task-conditioned OFT action head is trained on top of it with the encoder jointly fine-tuned, and
the policy is scored by closed-loop rollouts in the LIBERO simulator. The head is conditioned on
a 10-d task one-hot, not on language.

## Layout
```
inventory.py        # the pretraining recipe and the released artifact names
...
scripts/run_libero.sh      # pretrain -> train head -> evaluate, for one seed
results/            # the released checkpoint's eval log + make_table.py
...
`loss.grounding.target`. `inventory.py` records the exact overrides.
...
`--with-state` stores the 15-d robot-body state alongside the frames and actions; that column is
the grounding target.
...
## Reproduce the released run

```bash
export PY_PRETRAIN=.venv-pretrain/bin/python
export PY_LIBERO=.venv-libero/bin/python

libero/scripts/run_libero.sh 3072      # pretrain -> train head -> evaluate
```
...
`results/psgjepa.json` is the closed-loop evaluation of the released checkpoint: **90.8%** mean
success over the 10 tasks, 50 rollouts each. Break it down per task with:

```bash
python libero/results/make_table.py --per-task
```

The paper's Table 4 reports the mean over three training seeds; this release ships one of them,
so 90.8 is that seed's number rather than the table's.
...
within the eval spread noted above.
